-- AreaIQ Intelligence knowledge tables (database-backed, not hardcoded).
...
DROP POLICY IF EXISTS "Admin write area intelligence" ON public.area_intelligence;
CREATE POLICY "Admin write area intelligence"
  ON public.area_intelligence FOR ALL
  TO authenticated
  USING (EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role = 'admin'))
  WITH CHECK (EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role = 'admin'));

DROP POLICY IF EXISTS "Admin write builder intelligence" ON public.builder_intelligence;
CREATE POLICY "Admin write builder intelligence"
  ON public.builder_intelligence FOR ALL
  TO authenticated
  USING (EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role = 'admin'))
  WITH CHECK (EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role = 'admin'));
