...
export default function ProfileTab({ profile, onSave, onUploadLogo, onUploadAvatar }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      <p className="mt-1 text-sm text-neutral-500">Manage your account and business details.</p>
      ...
      <div className="mt-6 flex flex-wrap gap-6">
        ...
        <div className="rounded-2xl border border-neutral-200 bg-neutral-50 p-4 text-center">
          ...
          <div className="mx-auto mb-3 flex h-20 w-20 items-center justify-center overflow-hidden rounded-xl bg-white ring-2 ring-neutral-200">
            {profile.logo_url ? (
              // eslint-disable-next-line @next/next/no-img-element
          ...
          <input ref={logoRef} type="file" accept="image/*" hidden onChange={async (e) => {
            const f = e.target.files?.[0];
            if (f) await onUploadLogo(f);
          }} />
          <button type="button" style={btnSecondary} onClick={() => logoRef.current?.click()}>Upload Logo</button>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          ...
          <textarea style={{ ...inp, height: 88, resize: "vertical" }} value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} />
        </div>
      </div>
