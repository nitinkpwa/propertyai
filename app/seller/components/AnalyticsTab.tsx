  ...
  if (!analytics.hasData) {
    return (
      <div className="rounded-2xl border border-dashed border-neutral-200 bg-white px-6 py-16 text-center shadow-sm">
        ...
        <h3 className="text-lg font-semibold text-heading-primary">No analytics available yet</h3>
    ...
    <div>
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {metrics.map((m) => (
          ...
          >
            ...
            <p className="text-2xl font-bold text-heading-primary">{m.value}</p>
            <p className="mt-1 text-xs font-medium text-label">{m.label}</p>
          </div>
        ))}
      </div>
      ...
      {analytics.mostViewedProperty ? (
        <div className="mb-3 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wider text-muted">Most Viewed Property</p>
          <p className="mt-1 font-semibold text-heading-primary">{analytics.mostViewedProperty.title}</p>
      ...
      {analytics.mostSavedProperty ? (
        <div className="mb-3 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium uppercase tracking-wider text-muted">Most Saved Property</p>
          <p className="mt-1 font-semibold text-heading-primary">{analytics.mostSavedProperty.title}</p>
      ...
      {analytics.monthlyViews.length > 0 ? (
        <div className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
          <p className="mb-3 font-semibold text-heading-primary">Monthly Performance (Views)</p>
          {analytics.monthlyViews.map((row) => (
            <div key={row.month} className="flex justify-between border-b border-neutral-100 py-2 text-sm last:border-0">
              <span className="text-body">{row.month}</span>
