...
export default function BuyerDashboardPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setRecommended(Array.isArray(nextRecommended) ? nextRecommended : []);
        setUpcomingVisits(upcoming);
        setPendingApprovals(visits.filter((v) => v.status === "pending_approval").length);
        setCrmCounts({
          enquiriesCount: crm?.enquiriesCount ?? 0,
          savedCount: crm?.savedCount ?? 0,
          chatsCount: crm?.chatsCount ?? 0,
          visitsCount: crm?.visitsCount ?? 0,
          viewedCount: Array.isArray(nextRecent) ? nextRecent.length : 0,
        });
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 sm:space-y-8">
      ...
      {/* Metrics — horizontal snap on mobile */}
      ...
      {/* Investment opportunities shortcut */}
