  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted">Your assigned buyers, properties, and pipeline at a glance.</p>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => (
          ...
          >
            ...
            <p className="text-3xl font-bold tracking-tight text-heading-primary">{card.value}</p>
            <p className="mt-1 text-sm font-semibold text-heading-secondary">{card.label}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  if (buyers.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-neutral-200 bg-white py-16 text-center">
        <p className="text-sm text-muted">No buyers assigned to you yet.</p>
      </div>
    );
  }
      ...
      {buyers.map((buyer) => (
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <h3 className="font-semibold text-heading-primary">{buyer.full_name ?? "Buyer"}</h3>
            <LeadTemperatureBadge temperature={buyer.lead_temperature} score={buyer.lead_score} />
          </div>
          <p className="mt-1 text-sm text-muted">{buyer.phone ?? "—"}</p>
          <dl className="mt-4 space-y-2 text-xs">
            ...
            <div className="flex justify-between gap-2">
              <dt className="text-muted">Preferred Area</dt>
              <dd className="text-right text-body">{(buyer.preferred_locations ?? []).slice(0, 2).join(", ") || "—"}</dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-muted">Property Type</dt>
              <dd className="text-right capitalize text-body">{(buyer.preferred_property_types ?? []).join(", ") || "—"}</dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-muted">Purpose</dt>
              <dd className="capitalize text-body">{buyer.buying_purpose ?? "—"}</dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-muted">Timeline</dt>
              <dd className="capitalize text-body">{buyer.buying_timeline ?? "—"}</dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-muted">Lead Status</dt>
              <dd className="capitalize text-body">{buyer.lead_status ?? "—"}</dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-muted">Visit Status</dt>
              <dd className="capitalize text-body">{buyer.visit_status ?? "—"}</dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="text-muted">Last Chat</dt>
          ...
          {buyer.buyer_notes ? (
            <p className="mt-3 rounded-xl bg-neutral-50 px-3 py-2 text-xs text-body">{buyer.buyer_notes}</p>
          ) : null}
        </article>
      ))}
    </div>
...
export function ConnectPropertiesPanel({
  ...
  if (properties.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-neutral-200 bg-white py-16 text-center">
        <p className="text-sm text-muted">No properties assigned to you yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
        <h3 className="mb-4 font-semibold text-heading-primary">Monthly Activity</h3>
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
      <div className="mb-6 flex items-center justify-between">
        <h3 className="text-lg font-semibold text-heading-primary">Partner Settings</h3>
